    ...
    function formCreateArtwork(object $artwork=null){ ?>
        <form action="../_controller/studentViewController.php?action=<?php if($artwork==null || empty($artwork)){echo "create";}else{echo "edit";} ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="synopsis_court" rows="3" name="short_synopsis" ><?php if(!empty($artwork) || $artwork != null){ echo $artwork->getSynopsisShort() ;}?></textarea>
            </div>
            <div class="form-group">
                <label for="synopsis_long">Synopsis long</label>
                <textarea class="form-control" id="synopsis_long" rows="5" name="long_synopsis" ><?php if(!empty($artwork) || $artwork != null){ echo $artwork->getSynopsisLong() ;} ?></textarea>
            </div>
            <div class="form-group">
                <label for="thanks">Remerciements</label>
                <textarea class="form-control" id="thanks" rows="3" name="thanks" ><?php if(!empty($artwork) || $artwork != null){ echo $artwork->getThanks() ;} ?></textarea>
            </div>
            <div class="form-group">
